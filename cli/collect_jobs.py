...
def _title_matches_speedyapply_position(title: str, position: str) -> bool:
    ...
    """
    title_words = {w for w in re.findall(r"[a-z]+", title.lower()) if w not in _SPEEDYAPPLY_STOPWORDS and len(w) >= 2}
    if not title_words:
        return True
    position_l = position.lower()
    return all(re.search(rf"\b{re.escape(w)}\b", position_l) for w in title_words)


async def collect_speedyapply(title: str, existing_jobs: dict, profile: dict, max_jobs: int = 0, filters: dict | None = None) -> list[dict]:
    ...
    try:
        ...
        for row in rows:
            ...
            if not url or url in jobs or url in found_urls:
                continue
            if not _is_us_location(location, profile):
                print(f"    ⏭️  Skipped non-US location: {position} — {location}")
                continue
            ok_auth, reason = _matches_authorization_constraints(f"{position} {company} {location}", profile)
            if not ok_auth:
                print(f"    ⏭️  {reason}: {position} at {company}")
                continue
